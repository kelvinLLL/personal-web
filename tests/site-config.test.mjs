...
test('local dev origin uses the unified root port', () => {
  assert.equal(LOCAL_DEV_ORIGIN, 'http://127.0.0.1:3000');
...
test('proxy routing sends requests to the expected upstream dev server', () => {
  assert.equal(resolveProxyTarget('/'), `http://127.0.0.1:${PORTAL_DEV_PORT}`);
  assert.equal(resolveProxyTarget('/book-reader/'), `http://127.0.0.1:${BOOK_READER_DEV_PORT}`);
  assert.equal(resolveProxyTarget('/book-reader/assets/index.js'), `http://127.0.0.1:${BOOK_READER_DEV_PORT}`);
  assert.equal(resolveProxyTarget('/daily-nuance/'), `http://127.0.0.1:${DAILY_NUANCE_DEV_PORT}`);
  assert.equal(resolveProxyTarget('/daily-nuance/daily/latest'), `http://127.0.0.1:${DAILY_NUANCE_DEV_PORT}`);
});
